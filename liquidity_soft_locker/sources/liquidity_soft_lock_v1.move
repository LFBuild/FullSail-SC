/// Liquidity SoftLocker Module V1 without reward
/// 
...
module liquidity_soft_locker::liquidity_soft_lock_v1 {
    ...
    /// 
    /// This function creates and initializes the main SoftLocker object with default values,
    ...
    fun init(ctx: &mut sui::tx_context::TxContext) {
        ...
        transfer::share_object<SoftLocker>(locker);
    ...
    public fun get_ignore_whitelist_flag(locker: &SoftLocker): bool {
    ...
    public fun lock_position<CoinTypeA, CoinTypeB>(
        ...
        let duration_block = soft_time_manager::epoch_to_seconds(locker.periods_blocking[block_period_index]);
        let current_time = clock.timestamp_ms() / 1000;
        let expiration_time = soft_time_manager::epoch_next(current_time + duration_block);
        let full_unlocking_time = expiration_time + soft_time_manager::epoch_to_seconds(locker.periods_post_lockdown[block_period_index]);
        ...
        while (i < tranches.length()) {
            ...
            let liquidity_in_token = if (volume_in_coin_a) {
                soft_locker_utils::calculate_position_liquidity_in_token_a(pool, position_id_copy)
            } else {
                soft_locker_utils::calculate_position_liquidity_in_token_b(pool, position_id_copy)
            };
    ...
    public fun remove_lock_liquidity<CoinTypeA, CoinTypeB>(
        ...
        let mut remove_liquidity_amount = if (full_remove) {
            ...
            let number_epochs_after_expiration = soft_time_manager::number_epochs_in_timestamp(current_time - lock_position.lock_liquidity_info.last_remove_liquidity_time);
            ...
            integer_mate::full_math_u128::mul_div_floor(
                ...
                soft_time_manager::number_epochs_in_timestamp(lock_position.full_unlocking_time - lock_position.expiration_time) as u128,
            )
        };
        ...
        if (full_remove) {
            ...
            lock_position.lock_liquidity_info.last_remove_liquidity_time = soft_time_manager::epoch_start(current_time);
    ...
    /// Completely unlocks a position and destroys the SoftLockedPosition object.
    ...
    public fun split_position<CoinTypeA, CoinTypeB>(
        ...
        clock: &sui::clock::Clock,
        ctx: &mut TxContext
    ): (SoftLockedPosition<CoinTypeA, CoinTypeB>, SoftLockedPosition<CoinTypeA, CoinTypeB>) {
        checked_package_version(locker);
        let current_time = clock.timestamp_ms() / 1000;
        assert!(!locker.pause, ELockManagerPaused);
    ...
    /// 2. Destroys the SoftLockedPosition
    /// 3. Removes the position from the locker
    /// 
    /// # Arguments
    /// * `locker` - SoftLocker object
    /// * `lock_position` - SoftLockedPosition to destroy
    /// * `clock` - Clock object
    /// * `ctx` - Transaction context
    /// 
    /// # Returns
   ...
   public(package) fun lock_position_migrate<CoinTypeA, CoinTypeB>(
    ...
    /// 
    /// This function extracts the necessary information from a locked position to create a new SoftLockedPosition.
    /// 
    /// # Arguments
    /// * `lock_position` - SoftLockedPosition to extract information from
    /// 
    /// # Returns
    ...
    public(package) fun admins(locker: &SoftLocker): &sui::vec_set::VecSet<address> {
        &locker.admins
    }
