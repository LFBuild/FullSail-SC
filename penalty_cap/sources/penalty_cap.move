module penalty_cap::penalty_cap {
    
    const ENotOwner: u64 = 923752748582334234;
    
    public struct PENALTY_CAP has drop {} 

    public struct CreateCap has store, key {
        id: sui::object::UID,
    }

    public struct PenaltyCap has store, key {
        id: sui::object::UID,
    }

    public fun create_penalty_cap(
    ...
    fun init(penalty_cap_instance: PENALTY_CAP, ctx: &mut sui::tx_context::TxContext) {
        sui::package::claim_and_keep<PENALTY_CAP>(penalty_cap_instance, ctx);
        let new_cap = CreateCap { id: sui::object::new(ctx) };
