/*module full_sail::router {
    ...
    public fun swap_router(
        ...
        min_output_amount: u64,
        ctx: &mut TxContext
    ): (Coin<COIN_WRAPPER>, vector<CoinMetadata<COIN_WRAPPER>>) {
        let mut token_count = vector::length(&intermediary_tokens);
        let mut current_amount = input_amount;
        let mut tokens = intermediary_tokens;
        
        while(token_count > 0) {
            let next_token_ref = vector::borrow(&tokens, token_count - 1);
    ...
    public entry fun swap_route_entry_both_coins<BaseType, QuoteType>(
        ...
        let base_coin = exact_withdraw<BaseType>(input_amount, store, ctx);
        let base_metadata = coin_wrapper::get_wrapper<BaseType>(store);

        let (output_coin, tokens) = swap_router(
            pool,
            base_coin,
            base_metadata,
            intermediary_tokens,
        ...
        exact_deposit<QuoteType>(
            recipient,
            coin_wrapper::unwrap<QuoteType>(store, output_coin)
        );
