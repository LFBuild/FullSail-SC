#[test_only]
module liquidity_locker::liquidity_lock_v2_tests {
    ...
    fun test_lock_position() {
        ...
        scenario.next_tx(admin);
        {
            ...
            let mut locked_positions = liquidity_lock_v2::lock_position<TestCoinB, TestCoinA>(
                &global_config,
                &mut vault,
                &distribution_config,
                &mut locker,
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EInvalidBlockPeriodIndex)]
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EInvalidProfitabilitiesLength)]
    ...
    fun test_split_position() {
        ...
        scenario.next_tx(admin);
        {
            ...
            let (locked_position_11, locked_position_12, staking_reward_1) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
            ...
            let (locked_position_111, locked_position_112, staking_reward_2) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    #[test]
    fun test_claim_reward_lock_position_in_the_end_of_epoch() {
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)*9*1000/10);
        // create second lock in the end of epoch
        scenario.next_tx(admin);
        {
            let locker_create_cap = scenario.take_from_sender<locker_cap::CreateCap>();
            let gauge_create_cap = scenario.take_from_sender<gauge_cap::gauge_cap::CreateCap>();
        ...
        // Advance to Epoch 2 (OSAIL2)
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)*1000/10); // next epoch (2)
    ...
    #[test]
    fun test_claim_multiple_epoch_rewards() {
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        // Create  tranche and add reward for the first epoch
        // Add multiple rewards to the tranche for the first epoch
        // Create a new position
        scenario.next_tx(admin);
        {
            let locker_create_cap = scenario.take_from_sender<locker_cap::CreateCap>();
    ...
    #[test]
    fun test_claim_rewards_for_previous_epochs() {
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        // add reward to tranche for the first epoch in SAIL and claim it
        scenario.next_tx(admin);
        {
            let locker_create_cap = scenario.take_from_sender<locker_cap::CreateCap>();
            let gauge_create_cap = scenario.take_from_sender<gauge_cap::gauge_cap::CreateCap>();
            ...
            let tranche_id = sui::object::id<pool_tranche::PoolTranche>(tranche1);
            ...
            liquidity_lock_v2::collect_reward_sail<TestCoinB, TestCoinA, OSAIL1, SailCoinType>(
                &locker,
                &mut tranche_manager,
                &mut ve,
                &mut gauge,
        ...
        // add reward to tranche for the second epoch and claim it
        scenario.next_tx(admin);
        {
            let locker_create_cap = scenario.take_from_sender<locker_cap::CreateCap>();
            let gauge_create_cap = scenario.take_from_sender<gauge_cap::gauge_cap::CreateCap>();
    ...
    #[test]
    fun test_claim_rewards_after_split() { // claim after split
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        // split position in the middle of epoch 2
        ...
        scenario.next_tx(admin);
        {
            ...
            let (locked_position_1, locked_position_2, staking_reward) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault, 
        ...
        // Advance to Epoch 3 (OSAIL3)
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)/2*1000); // next epoch (3)
        ...
        scenario.next_tx(admin);
        {
            ...
            // despite the fact that for half of the epoch the position was not split, the total reward is divided 60/40
            assert!(lock1.get_amount() == 599999, 9262236263635);
        ...
        scenario.next_tx(admin);
        {
            ...
            assert!(reward3_1.value() + reward3_2.value() == 999998, 9234129832754); // 10% of total reward for epoch 3 
    ...
    fun test_split_position_pause(){
        ...
        scenario.next_tx(admin);
        {
            ...
            let (locked_position_11, locked_position_12, staking_reward) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EFullLockPeriodEnded)]
    ...
    #[test]
    fun test_split_position_in_lineal_unlock(){
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            let (staked_position_user) = create_and_deposit_position<TestCoinB, TestCoinA>(
                &mut scenario,
                &global_config,
                &distribution_config,
                &mut gauge,
            ...
            let (staked_position_user_2) = create_and_deposit_position<TestCoinB, TestCoinA>(
                &mut scenario,
                &global_config,
                &distribution_config,
                &mut gauge,
            ...
            transfer::public_transfer(staked_position_user, admin);
            transfer::public_transfer(staked_position_user_2, admin);
            ...
            assert!(full_unlocking_time_1 == (distribution::common::epoch_start(clock.timestamp_ms()/1000) + 3*86400*7), 9234326345);
        ...
        // set_total_incomed_and_add_reward for epoch 3
        scenario.next_tx(admin);
        {
            let mut tranche_manager = scenario.take_shared<pool_tranche::PoolTrancheManager>();
            let pool = scenario.take_from_sender<pool::Pool<TestCoinB, TestCoinA>>();
        ...
        // split position in lineal unlock
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)/2*1000);
        scenario.next_tx(admin);
        {
            let voter = scenario.take_shared<voter::Voter>();
            ...
            assert!(reward1.value() == 4104558, 9234129832754);
            transfer::public_transfer(sui::coin::from_balance(reward1, scenario.ctx()), admin);
            ...
            let (locked_position_21, locked_position_22, staking_reward) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL3>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
        ...
        scenario.next_tx(admin);
        {
            ...
            pool_tranche::set_total_incomed_and_add_reward<OSAIL4, RewardCoinType3>(
                &mut tranche_manager,
                sui::object::id<clmm_pool::pool::Pool<TestCoinB, TestCoinA>>(&pool),
                sui::object::id<pool_tranche::PoolTranche>(tranche1),
                clock.timestamp_ms()/1000,
        ...
        // full remove and unlock position for epoch 4
        scenario.next_tx(admin);
        {
            let global_config = scenario.take_shared<config::GlobalConfig>();
            let distribution_config = scenario.take_shared<distribution_config::DistributionConfig>();
            ...
            assert!(reward3.value() == 4354526/2, 9234129832757);
            transfer::public_transfer(sui::coin::from_balance(reward3, scenario.ctx()), admin);
            ...
            transfer::public_transfer(staking_reward3, scenario.sender());

            // full unlock
            let (remove_balance_a, remove_balance_b, staking_reward) = liquidity_lock_v2::remove_lock_liquidity<TestCoinB, TestCoinA, SailCoinType, OSAIL4>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    fun test_remove_lock_liquidity(){
        ...
        //     let reward4 = sui::coin::mint_for_testing<RewardCoinType4>(10000000, scenario.ctx());
        //     pool_tranche::set_total_incomed_and_add_reward<OSAIL5, RewardCoinType4>(
        //         &mut tranche_manager,
        //         sui::object::id<clmm_pool::pool::Pool<TestCoinB, TestCoinA>>(&pool),
        //         sui::object::id<pool_tranche::PoolTranche>(tranche1),
        //         clock.timestamp_ms()/1000,
        ...
        scenario.next_tx(admin);
        {
            ...
            assert!(reward4.value() == 7219319, 9234129832756);
            ...
            // full unlock
            let (remove_balance_a, remove_balance_b, staking_reward) = liquidity_lock_v2::remove_lock_liquidity<TestCoinB, TestCoinA, SailCoinType, OSAIL5>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
        ...
        clock::increment_for_testing(&mut clock, common::get_time_to_finality_ms()+1);
        scenario.next_tx(admin);
        {
            let mut ve = scenario.take_shared<voting_escrow::VotingEscrow<SailCoinType>>();
    ...
    fun test_last_claim_reward(){
        ...
        scenario.next_tx(admin);
        {
            ...
            transfer::public_transfer(staked_position_user, user);
            transfer::public_transfer(staked_position_user_2, user);
        ...
        scenario.next_tx(admin);
        {
            ...
            let reward4 = sui::coin::mint_for_testing<RewardCoinType3>(10000000, scenario.ctx());
            pool_tranche::set_total_incomed_and_add_reward<OSAIL5, RewardCoinType3>(
                &mut tranche_manager,
                sui::object::id<clmm_pool::pool::Pool<TestCoinB, TestCoinA>>(&pool),
                sui::object::id<pool_tranche::PoolTranche>(tranche1),
                clock.timestamp_ms()/1000,
        ...
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)/2*1000);
        // claim rewards for staking for 1, 2
        scenario.next_tx(admin);
        {
            let voter = scenario.take_shared<voter::Voter>();
            let mut minter = scenario.take_shared<minter::Minter<SailCoinType>>();
        ...
        //     let locker = scenario.take_shared<liquidity_lock_v2::Locker>();
        //     let mut ve = scenario.take_shared<voting_escrow::VotingEscrow<SailCoinType>>();
        //     let mut staked_position = scenario.take_from_sender<distribution::gauge::StakedPosition>();
        //     let distribution_config = scenario.take_shared<distribution_config::DistributionConfig>();

        //     let staking_reward = distribution::minter::get_position_reward<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
        //         &mut minter,
        //         &voter,
        //         &distribution_config,
        //         &mut gauge,
        ...
        scenario.next_tx(admin);
        {
            ...
            assert!(reward3.value() == 4354526, 9234129832757);
            transfer::public_transfer(sui::coin::from_balance(reward3, scenario.ctx()), admin);
            ...
            transfer::public_transfer(staking_reward4, scenario.sender());
        ...
        // full remove and unlock position for epoch 5
        scenario.next_tx(admin);
        {
            let global_config = scenario.take_shared<config::GlobalConfig>();
            let distribution_config = scenario.take_shared<distribution_config::DistributionConfig>();
    ...
    fun test_remove_lock_liquidity_by_epoch(){
        ...
        scenario.next_tx(admin);
        {
            ...
            assert!(reward4.value() == 3464056, 9234129832758);
            transfer::public_transfer(sui::coin::from_balance(reward4, scenario.ctx()), admin);
        ...
        scenario.next_tx(admin);
        {
            ...
            let reward5 = liquidity_lock_v2::collect_reward<TestCoinB, TestCoinA, OSAIL5, SailCoinType, RewardCoinType3>(
                ...
                &mut pool,
                &mut locked_position_2,
                common::epoch_start(common::epoch_to_seconds(5)),
                &clock,
                scenario.ctx()
            );

            assert!(reward5.value() == 2844002, 9234129832758);
            transfer::public_transfer(sui::coin::from_balance(reward5, scenario.ctx()), admin);
            ...
            // full unlock
            let (remove_balance_a, remove_balance_b, staking_reward) = liquidity_lock_v2::remove_lock_liquidity<TestCoinB, TestCoinA, SailCoinType, OSAIL6>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    fun test_unlock_position_without_remove_liquidity(){
        ...
        #[test]
    fun test_unlock_position_without_linear_unlock(){
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            std::vector::push_back(&mut periods_blocking, 6);
            let mut periods_post_lockdown = std::vector::empty();
            std::vector::push_back(&mut periods_post_lockdown, 0);
            std::vector::push_back(&mut periods_post_lockdown, 0);
            std::vector::push_back(&mut periods_post_lockdown, 0);
            liquidity_lock_v2::init_locker(
                &admin_cap,
        ...
        scenario.next_tx(admin);
        {
            ...
            // full unlock without unlock period
            let (staked_position, balance_a, balance_b) = liquidity_lock_v2::unlock_position<TestCoinB, TestCoinA>(
                &mut locker,
                locked_position_1,
                &mut gauge,
                &clock
            ...
            assert!(!locker.is_position_locked(position_id), 9234887456444);
            transfer::public_transfer(sui::coin::from_balance(balance_a, scenario.ctx()), scenario.sender());
            transfer::public_transfer(sui::coin::from_balance(balance_b, scenario.ctx()), scenario.sender());
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::ERewardsNotCollected)]
    ...
    #[test]
    #[expected_failure(abort_code = pool_tranche::EEmptyEpochOSailToken)]
    fun test_claimed_rewards_before_lock_time(){
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EClaimEpochIncorrect)]
    fun test_claimed_rewards_after_last_reward_claim_epoch(){
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            tranche_manager.update_osail_epoch<OSAIL3>(
                common::epoch_to_seconds(3),
                scenario.ctx()
            );
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EClaimEpochIncorrect)]
    fun test_claimed_rewards_after_expiration_time(){
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            let reward1 = sui::coin::mint_for_testing<SailCoinType>(10000000, scenario.ctx());
            pool_tranche::set_total_incomed_and_add_reward<OSAIL1, SailCoinType>(
                &mut tranche_manager,
                sui::object::id<clmm_pool::pool::Pool<TestCoinB, TestCoinA>>(&pool),
                sui::object::id<pool_tranche::PoolTranche>(tranche1),
                clock.timestamp_ms()/1000,
        ...
        scenario.next_tx(admin);
        {
            ...
            let reward2 = sui::coin::mint_for_testing<RewardCoinType2>(10000000, scenario.ctx());
            pool_tranche::set_total_incomed_and_add_reward<OSAIL2, RewardCoinType2>(
                &mut tranche_manager,
                sui::object::id<clmm_pool::pool::Pool<TestCoinB, TestCoinA>>(&pool),
                sui::object::id<pool_tranche::PoolTranche>(tranche1),
                clock.timestamp_ms()/1000,
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EFullLockPeriodNotEnded)]
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::ELockPeriodNotEnded)]
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::ENoLiquidityToRemove)]
    ...
    fun test_change_tick_range_with_swap_a2b() {
        ...
        scenario.next_tx(admin);
        {
            ...
            let staking_reward_11 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    fun test_change_tick_range_with_return_remaining_to_position() { // changing interval with returning remaining to position
        ...
        scenario.next_tx(admin);
        {
            ...
            // b2a
            let staking_reward_12 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL1>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    fun test_claim_rewards_after_change_tick_range_in_the_end_of_epoch() { // claim after change tick range in the end of epoch
        ...
        scenario.next_tx(admin);
        {
            ...
            let staking_reward_11 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
        ...
        scenario.next_tx(admin);
        {
            ...
            let staking_reward_11 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL3>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::ERewardsNotCollected)]
    fun test_rewards_not_collected_when_change_tick_range() {
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            clock::increment_for_testing(&mut clock, 3600*7*24*2*1000);
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::ENotChangedTickRange)]
    ...
    // Timeout on this test after resent update. We should change it to be smaller.
    #[test]
    fun test_claim_rewards_after_change_tick_range_and_split_positions() { // claim after changes tick range and splits positions
        ...
        scenario.next_tx(admin);
        {
            ...
            let (mut locked_position_1, locked_position_2, staking_reward_1) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
            ...
            position_ids.add(1,  sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_1));
            position_ids.add(2,  sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_2));
        ...
        scenario.next_tx(admin);
        {
            ...
            let mut stats = scenario.take_shared<clmm_pool::stats::Stats>();
            let price_provider = scenario.take_shared<clmm_pool::price_provider::PriceProvider>();
            let mut locked_position2 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(); // 67%
            let mut locked_position1 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(); // 33%
            ...
            let staking_reward_12 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
        ...
        scenario.next_tx(admin);
        {
            ...
            let mut pool = scenario.take_from_sender<pool::Pool<TestCoinB, TestCoinA>>();
            let price_provider = scenario.take_shared<clmm_pool::price_provider::PriceProvider>();
            let locked_position2 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(); // 67%

            assert!(position_ids.borrow(2) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position2));

            let (locked_position_21, locked_position_22, staking_reward_2) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
            ...
            assert!(position_ids.borrow(2) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_21));
            position_ids.add(3,  sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_22));
        ...
        scenario.next_tx(admin);
        {
            ...
            let voter = scenario.take_shared<voter::Voter>();
            let price_provider = scenario.take_shared<clmm_pool::price_provider::PriceProvider>();
            let locked_position3 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            let locked_position2 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            let mut locked_position1 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
    ...
    #[test]
    fun test_claim_rewards_after_change_tick_range_and_split_position_eq_range() { // positions are in the same range after split
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        scenario.next_tx(admin);
        {
            ...
            let mut gauge = scenario.take_from_sender<gauge::Gauge<TestCoinB, TestCoinA>>();
            let mut pool = scenario.take_from_sender<pool::Pool<TestCoinB, TestCoinA>>();
            let stats = scenario.take_shared<clmm_pool::stats::Stats>();
            let price_provider = scenario.take_shared<clmm_pool::price_provider::PriceProvider>();
            let mut locked_position2 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(); // 67%
        ...
        scenario.next_tx(admin);
        {
            ...
            let (locked_position_31, locked_position_32, staking_reward_3) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
                ...
                &mut pool,
                locked_position3,
                50000, // 33.5% -> 16.75% + 16.75%
                &clock,
                scenario.ctx()
            );

            transfer::public_transfer(staking_reward_3, scenario.sender());

            assert!(position_ids.borrow(3) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_31));
            position_ids.add(4,  sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position_32));

            transfer::public_transfer(locked_position_31, admin);
        ...
        scenario.next_tx(admin);
        {
            ...
            let vault = scenario.take_shared<rewarder::RewarderGlobalVault>();
            let locker = scenario.take_shared<liquidity_lock_v2::Locker>();
            let mut locked_position4 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            let mut locked_position3 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            let mut locked_position2 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            let mut locked_position1 = scenario.take_from_sender<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>();
            ...
            assert!(position_ids.borrow(2) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position2));
            assert!(position_ids.borrow(3) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position3));
            assert!(position_ids.borrow(4) == sui::object::id<liquidity_lock_v2::LockedPosition<TestCoinB, TestCoinA>>(&locked_position4));
    ...
    #[test]
    fun test_claim_rewards_after_change_tick_range_and_split_position_no_eq_range() { // positions change ranges after split
        let admin = @0x1;
        let mut scenario = test_scenario::begin(admin);
        let mut clock = clock::create_for_testing(scenario.ctx());
        ...
        // store position IDs to identify them in subsequent transactions
        let mut position_ids =  sui::table::new<u32, ID>(scenario.ctx());
        // change range position and split in the 10/100 of epoch 2
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)*10*1000/100);
        scenario.next_tx(admin);
        {
            let locker_create_cap = scenario.take_from_sender<locker_cap::CreateCap>();
            ...
            let (mut locked_position_1, locked_position_2, staking_reward) = liquidity_lock_v2::split_position<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
                &mut vault,
            ...
            // For the first position, the range will be half as large
            // Therefore, the reward will be twice as large
            let staking_reward_11 = liquidity_lock_v2::change_tick_range<TestCoinB, TestCoinA, SailCoinType, OSAIL2>(
                &global_config,
                &distribution_config,
                &mut minter,
        ...
        // Advance to Epoch 3 (OSAIL3)
        clock::increment_for_testing(&mut clock, common::epoch_to_seconds(1)*90*1000/100); // next epoch (3)
    ...
    fun test_admins() {
        ...
        scenario.next_tx(admin);
        {
            ...
            liquidity_lock_v2::check_admin(&locker, admin2);

            liquidity_lock_v2::revoke_admin(
                &admin_cap,
                &mut locker,
                admin2,
                scenario.ctx()
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EAddressNotAdmin)]
    ...
    #[test]
    #[expected_failure(abort_code = liquidity_lock_v2::EAdminNotWhitelisted)]
    fun test_not_admin_locker_pause() {
        ...
        scenario.next_tx(admin);
        {
            ...
            liquidity_lock_v2::locker_pause(
                &mut locker,
                true,
                scenario.ctx()
            );
