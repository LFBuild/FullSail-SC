...
module governance::distribution_config {
    ...
    public fun get_liquidity_update_cooldown(distribution_config: &DistributionConfig): u64 {
        if (sui::bag::contains(&distribution_config.bag, b"liquidity_update_cooldown")) {
            *sui::bag::borrow(&distribution_config.bag, b"liquidity_update_cooldown")
        } else {
            0
        }
    }
    ...
    public(package) fun set_liquidity_update_cooldown(
        ...
        if (distribution_config.bag.contains(b"liquidity_update_cooldown")) {
            distribution_config.bag.remove<vector<u8>, u64>(b"liquidity_update_cooldown");
        };
        distribution_config.bag.add(b"liquidity_update_cooldown", new_cooldown);
    }
