...
fun test_settle_killed_gauge_rewards_continue_until_epoch_end() {
    ...
    // 7. Check that LP can still earn rewards after settle (still in current epoch)
    scenario.next_tx(lp);
    {
        let gauge = scenario.take_shared<Gauge<USD_TESTS, AUSD>>();
        let pool = scenario.take_shared<Pool<USD_TESTS, AUSD>>();
        ...
        // Position should have earned approximately 1/3 of the rewards so far
        assert!(gauge_base_emissions / 3 - earned <= 2, 1);
    ...
    // 9. Check that rewards continued to accrue until epoch end
    scenario.next_tx(lp);
    {
        let gauge = scenario.take_shared<Gauge<USD_TESTS, AUSD>>();
        let pool = scenario.take_shared<Pool<USD_TESTS, AUSD>>();
        ...
        let earned_at_end = minter::earned_by_position<USD_TESTS, AUSD, SAIL, OSAIL1>(
            &minter,
            &gauge,
            &pool,
            lp_position_id,
        ...
        assert!(gauge_base_emissions - earned_at_end <= 5, 2);
