...
module full_sail::router_test {
    ...
    fun test_unstake_and_remove_liquidity_both_coins_entry() {
        ...
        next_tx(scenario, OWNER);
        {
            ...
            let (optimal_a, optimal_b) = router::get_optimal_amounts_for_testing<USDT, SUI>(
                pool,
                &quote_metadata,
                &base_metadata,
                100000,
            ...
            next_tx(scenario, OWNER);
            let mut gauge1 = ts::take_shared<Gauge<USDT, SUI>>(scenario);
            ...
            next_tx(scenario, OWNER);
            let mut gauge2 = ts::take_shared<Gauge<USDT, SUI>>(scenario);
            let pool1 = gauge::liquidity_pool(&mut gauge2);
            let (coin_in, coin_out) = liquidity_pool::liquidity_amounts<USDT, SUI>(
                pool1,
                5000
            );
