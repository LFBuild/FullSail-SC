...
def generate_distribution_script(excel_file, output_script, package, treasury_cap, token_name, token_symbol, decimals):
    """
    ...
    Args:
        ...
        treasury_cap (str): The treasury cap object ID.
    ...
    with open(output_script, 'w') as f:
        ...
        f.write(f"export TREASURY_CAP_ID='{treasury_cap}'\n")
        ...
        for index, row in df.iterrows():
            ...
            mint_command = f"sui::coin::mint '<{token_name}>' @$TREASURY_CAP_ID {amount_adjusted}"
            # I am not sure about how to mint and transfer in the same command
            # please check the documentation
            # transfer_command = f"sui::transfer::public_transfer(coin, {address})"
            # transaction = f"sui client call --package $PACKAGE_ID --module {token_name} --function '{mint_command};{transfer_command}' --gas-budget 10000"
            coin_var = f"coin{index}"
            ptb_command += f" \\\n--move-call sui::coin::mint \"<$TOKEN_TYPE>\" @$TREASURY_CAP_ID {amount_adjusted}"
            ptb_command += f" \\\n--assign {coin_var}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--treasury-cap", required=True, help="The treasury cap object ID.")
